...
local Config = {
    ...
    MENU_Open = true,
    MENU_Tab = 1,
    MENU_Scale = 1.0,      -- Neue Option: GUI Skalierung
    MENU_ShowLogo = true    -- Neue Option: Logo anzeigen
}
...
local MenuItems = {
    ...
    -- Neue Menüoptionen für Skalierung und Logo
    {tab = 3, name = "UI SETTINGS", type = "label"},
    {tab = 3, name = "GUI Scale", key = "MENU_Scale", type = "slider", min = 0.6, max = 1.4, step = 0.05},
    {tab = 3, name = "Show Logo", key = "MENU_ShowLogo", type = "toggle"}
}
...
function GUI.UpdateScale()
    if not GUI.Frame then return end
    local scale = Config.MENU_Scale
    
    -- Skaliere das gesamte Menü
    GUI.Frame.Size = UDim2.new(0, 300 * scale, 0, 380 * scale)
    
    -- Aktualisiere Position, wenn nicht gezogen wird
    if not GUI.Dragging then
        local mousePos = UserInputService:GetMouseLocation()
        local framePos = GUI.Frame.AbsolutePosition
        if framePos.X > 0 and framePos.Y > 0 then
            -- Behalte relative Position bei
        end
    end
    
    -- Aktualisiere Logo Position und Größe wenn vorhanden
    if GUI.Logo then
        GUI.Logo.Size = UDim2.new(0, 48 * scale, 0, 48 * scale)
        GUI.Logo.Position = UDim2.new(0, 10 * scale, 0, 10 * scale)
        for _, text in ipairs(GUI.Logo:GetChildren()) do
...
function GUI.ToggleLogo()
    if GUI.Logo then
        GUI.Logo.Visible = Config.MENU_ShowLogo
    end
end
...
function GUI.Create()
    ...
    main.Size = UDim2.new(0, menuW * scale, 0, menuH * scale)
    main.Position = UDim2.new(0, 20, 0.5, -menuH * scale/2)
    main.Active = true
    ...
    logo.Name = "Logo"
    logo.BackgroundColor3 = Palette.MenuAccent
    logo.BackgroundTransparency = 0.2
    logo.BorderSizePixel = 0
    logo.Size = UDim2.new(0, 48 * scale, 0, 48 * scale)
    logo.Position = UDim2.new(0, 10 * scale, 0, 10 * scale)
    logo.Visible = Config.MENU_ShowLogo
    logo.Parent = main
    ...
    logoSub.Position = UDim2.new(0, 0, 1, -14 * scale)
    logoSub.Size = UDim2.new(1, 0, 0, 12 * scale)
    logoSub.Font = Enum.Font.RobotoMono
    logoSub.TextSize = 7 * scale
    logoSub.TextColor3 = Palette.MenuTextDim
    logoSub.Text = "XENO"
    ...
    title.Size = UDim2.new(1, 0, 0, titleH * scale)
    title.Parent = main
    
    local titleText = Instance.new("TextLabel")
    titleText.BackgroundTransparency = 1
    titleText.Position = UDim2.new(0, 10 * scale, 0, 0)
    titleText.Size = UDim2.new(0, 120 * scale, 1, 0)
    ...
    xenoTag.Position = UDim2.new(0, 100 * scale, 0, 0)
    xenoTag.Size = UDim2.new(0, 60 * scale, 1, 0)
    xenoTag.Font = Enum.Font.RobotoMono
    xenoTag.TextSize = 10 * scale
    ...
    hotkeyText.Position = UDim2.new(1, -90 * scale, 0, 0)
    hotkeyText.Size = UDim2.new(0, 80 * scale, 1, 0)
    hotkeyText.Font = Enum.Font.RobotoMono
    hotkeyText.TextSize = 10 * scale
    ...
    tabFrame.Position = UDim2.new(0, 0, 0, titleH * scale)
    tabFrame.Size = UDim2.new(1, 0, 0, tabH * scale)
    tabFrame.Parent = main
    
    local tabW = (menuW * scale) / #Tabs
    ...
    contentFrame.Position = UDim2.new(0, 8 * scale, 0, (titleH + tabH + 4) * scale)
    contentFrame.Size = UDim2.new(1, -16 * scale, 1, -(titleH + tabH + footerH + 8) * scale)
    ...
    layout.Padding = UDim.new(0, 2 * scale)
    layout.Parent = contentFrame
    
    local footer = Instance.new("Frame")
    footer.Name = "Footer"
    footer.BackgroundTransparency = 1
    footer.Position = UDim2.new(0, 0, 1, -footerH * scale)
    footer.Size = UDim2.new(1, 0, 0, footerH * scale)
    footer.Parent = main
    
    local credit = Instance.new("TextLabel")
    credit.BackgroundTransparency = 1
    credit.Position = UDim2.new(0, 10 * scale, 0, 0)
    credit.Size = UDim2.new(1, -20 * scale, 1, 0)
    credit.Font = Enum.Font.RobotoMono
    credit.TextSize = 11 * scale
...
function GUI.UpdateContent()
    ...
    for _, menuItem in ipairs(MenuItems) do
        if menuItem.tab == Config.MENU_Tab then
            ...
            item.Size = UDim2.new(1, 0, 0, itemH * scale)
            item.LayoutOrder = order
            item.Parent = GUI.ContentFrame
            
            local label = Instance.new("TextLabel")
            label.BackgroundTransparency = 1
            label.Position = UDim2.new(0, 8 * scale, 0, 0)
            label.Size = UDim2.new(0.6, 0, 0, 24 * scale)
            label.Font = Enum.Font.RobotoMono
            label.TextSize = (menuItem.type == "label" and 12 or 11) * scale
            ...
            if menuItem.type == "toggle" then
                ...
                toggle.Position = UDim2.new(1, -40 * scale, 0.5, -7 * scale)
                toggle.Size = UDim2.new(0, 28 * scale, 0, 14 * scale)
                toggle.Parent = item
                Instance.new("UICorner", toggle).CornerRadius = UDim.new(0, 7 * scale)
                
                local knob = Instance.new("Frame")
                knob.Name = "Knob"
                knob.BackgroundColor3 = Color3.new(1, 1, 1)
                knob.BorderSizePixel = 0
                knob.Position = Config[menuItem.key] and UDim2.new(1, -12 * scale, 0.5, -5 * scale) or UDim2.new(0, 2 * scale, 0.5, -5 * scale)
                knob.Size = UDim2.new(0, 10 * scale, 0, 10 * scale)
                knob.Parent = toggle
                ...
                btn.MouseButton1Click:Connect(function()
                    ...
                    -- Spezielle Behandlung für Skalierung
                    if menuItem.key == "MENU_Scale" then
                        GUI.UpdateScale()
                    elseif menuItem.key == "MENU_ShowLogo" then
                        GUI.ToggleLogo()
                    end
                end)
            ...
            elseif menuItem.type == "slider" then
                ...
                valLabel.Position = UDim2.new(1, -50 * scale, 0, 0)
                valLabel.Size = UDim2.new(0, 45 * scale, 0, 20 * scale)
                valLabel.Font = Enum.Font.RobotoMono
                valLabel.TextSize = 11 * scale
                ...
                track.Position = UDim2.new(0, 8 * scale, 0, 24 * scale)
                track.Size = UDim2.new(1, -16 * scale, 0, 6 * scale)
                track.Parent = item
                Instance.new("UICorner", track).CornerRadius = UDim.new(0, 3 * scale)
                ...
                fill.Parent = track
                Instance.new("UICorner", fill).CornerRadius = UDim.new(0, 3 * scale)
    ...
    for _, item in ipairs(GUI.Items) do
        totalH = totalH + item.AbsoluteSize.Y + 2 * scale
    end
